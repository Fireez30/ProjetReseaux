...
printf("taille renvoyée par stat : %ld\n",(long)sb.st_size);
...
int res1=send(dSock,&taille,sizeof(long),0);// j'envoie la taille totale du fichier au serveur
if (res1 == -1){
	printf("Erreur lors de l'envoi !");
	return -1;
}

int taille1=(int)(strlen(adressefic));//taille du nom du fichier a envoyer 
...
while (acclus < taille){// tant qu'on peut lire dans le fichier
	...
	int nblus = fread(&buffer,1,100,fp);// je lis jusqua 100 caractere dans le fichier
	if (nblus == -1){
